...
client = OpenAI(
    ...
    api_key= "voc-1210698298159874496027169ea4ee1eeea88.28908859")
...
def routing_agent(query, *args):
    ...
    classification_system_prompt = f"""You are a helpful AI assistant that categorizes retail-related user queries. 
    ...
    task_type = call_openai(classification_system_prompt, classification_user_prompt).strip().lower()
    ...
    if task_type == "product research":
        ...
        product_research = product_researcher_agent(query)
        customer_analysis = customer_analyzer_agent(query)
        return pricing_strategist_agent(query, product_research, customer_analysis)

    else:
        # fallback se il classificatore risponde qualcosa di inatteso
        print(f"Unrecognized classification '{task_type}'. Defaulting to Product Research.")
        return product_researcher_agent(query)
